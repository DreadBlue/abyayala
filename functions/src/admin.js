...
const adminBookings = onCall(async (request) => {
  ...
  const activities = Object.entries(filters.activities)
    .filter(([key, value]) => value === true)
    .map(([key]) => key);
  const horarios = Object.entries(filters.horario)
    .filter(([key, value]) => value === true)
    .map(([key]) => key);
  const comida = Object.entries(filters.comida)
    .filter(([key, value]) => value === true)
    .map(([key]) => key);
  const transporte = Object.entries(filters.transporte)
    .filter(([key, value]) => value === true)
    .map(([key]) => key);
...
const createRequest = onCall(async (request) => {
  ...
  const reservaQuery = db
    .collection('reservas')
    .where('idReserva', '==', clientObject.id)
    .where('Correo', '==', clientObject.Correo);
  ...
  try {
    ...
    for (const key in clientObject) {
      if (clientObject[key] !== serverObject[key]) {
        differences[key] = {
          client: clientObject[key],
          server: serverObject[key],
        };
        if (tipo === '') {
          tipo = key;
        } else {
    ...
    for (const difference in differences) {
      if (Object.prototype.hasOwnProperty.call(differences, difference)) {
        solicitudes[solicitud] = {
          solicitud: `Cambiar ${difference.toLocaleLowerCase} de ${differences[difference].client} a ${differences[difference].server}`,
        };
        solicitud++;
      }
    }
...
const fetchBill = async (id) => {
  try {
    const fileName = `comprobantes/${id}`;
    const options = {
      version: 'v4',
      action: 'read',
